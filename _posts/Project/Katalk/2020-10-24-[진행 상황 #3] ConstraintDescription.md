...
혹시 `message` 의 값이 변경되지 않은 것인지 확인해보기 위하여 `@Pattern` 의 `message` 값을 불러오는 코드를 까서 직접 구현해 보았습니다. 다음은 그 예시입니다.
...
저는 그런 상황을 원치 않으므로 `PropertyResourceBundle` 의 HashMap 타입의 속성인 lookup에 내가 설정한 키와 밸류를 집어넣어야 합니다.

여기서 벽에 막혀서 진도를 못나가고 있습니다. 언젠가 해결이 된다면 이어서 포스팅하겠습니다.